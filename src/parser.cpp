...
auto CommandLineParser::parse(std::span<char* const> raw_arguments) -> std::expected<SimulationConfig, std::string> {
  ...
  if (!parsed_generations)    { return std::unexpected("Invalid <generations>: "    + parsed_generations.error()); }
  if (!parsed_grid_dimension) { return std::unexpected("Invalid <grid_dimension>: " + parsed_grid_dimension.error()); }
  if (!parsed_density)        { return std::unexpected("Invalid <density>: "        + parsed_density.error()); }
  if (!parsed_seed)           { return std::unexpected("Invalid <seed>: "           + parsed_seed.error()); }
  ...
  if (number_of_generations < kMinimumPositive) { return std::unexpected("<generations> must be >= 1"); }
  if (grid_dimension        < kMinimumPositive) { return std::unexpected("<grid_dimension> must be >= 1"); }
  if (!std::isfinite(initial_density) ||
      initial_density < kMinimumDensityInclusive ||
