...
void Simulation::free_grid(char ***g, int N) {
  ...
  for (int x = 0; x < N; ++x) {
    if (g[x]) {
      if (g[x][0])
        std::free(g[x][0]); // contiguous N*N block
...
void Simulation::run() {
  ...
  // - On ties for maxima, keep the lowest generation (update only on strictly
  // greater).

  // Placeholder: count initial as generation 0 only (so you can compile/run
  // now).
