...
void Simulation::run() {
  ...
  for (int gen = 1; gen <= generations_; ++gen) {
    ...
    // counts[...] default-initialized to 0
    ...
    for (int z = 0; z < n; ++z) {
      for (int y = 0; y < n; ++y) {
        for (int x = 0; x < n; ++x) {
          ...
          // Single 3x3x3 sweep, count all non-zero neighbors (any species).
          ...
          if (total_neighbors >= 5 && total_neighbors <= 13) {
            // cell continues to be whatever it was (alive stays that species, dead stays dead)
            new_value = current;
          } else {
