...
// Local encoding for "dead" cells in this TU.
constexpr unsigned char DEAD_CELL = 0;
...
int alive_neighbors_bitmask(int                                        x,
                            int                                        y,
                            int                                        z,
                            int                                        grid_dimension,
                            int                                        wrap_mask,
    ...
    for (int dy = -1; dy <= 1; ++dy)
    {
      ...
      for (int dx = -1; dx <= 1; ++dx)
      {
        ...
        const std::size_t idx =
            (static_cast<std::size_t>(z2) * n_sz + static_cast<std::size_t>(y2)) * n_sz
...
template <bool CountSpecies>
int alive_neighbors_modular(int                                        x,
                            int                                        y,
                            int                                        z,
                            int                                        grid_dimension,
                            const std::vector<unsigned char>&          grid,
...
template <bool CountSpecies>
int alive_neighbors_common(int                                        x,
                           int                                        y,
                           int                                        z,
                           int                                        grid_dimension,
                           bool                                       use_bitmask_wrap,
                           int                                        wrap_mask,
                           const std::vector<unsigned char>&          grid,
                           std::array<int, generator::N_SPECIES + 1>& species_counts)
{
  ...
  if (use_bitmask_wrap)
  {
    ...
    return alive_neighbors_bitmask<CountSpecies>(x, y, z, grid_dimension, wrap_mask, grid,
                                                 species_counts);
  }
...
{
  // Dummy array is never used when CountSpecies == false (pruned by if constexpr).
  std::array<int, generator::N_SPECIES + 1> dummy{};
  return sim_detail::alive_neighbors_common<false>(x, y, z, grid_dimension_, use_bitmask_wrap_,
                                                   wrap_mask_, grid_, dummy);
}

// Dead-cell path: total + per-species counts, early-exit at >10 neighbors.
int Simulation::alive_neighbors_with_species(
    int                                        x,
    int                                        y,
    int                                        z,
    std::array<int, generator::N_SPECIES + 1>& species_counts) const
{
  return sim_detail::alive_neighbors_common<true>(x, y, z, grid_dimension_, use_bitmask_wrap_,
                                                  wrap_mask_, grid_, species_counts);
}
